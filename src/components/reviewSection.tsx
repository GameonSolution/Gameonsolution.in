...
const ReviewsSection = () => {
  ...
  return (
    <section
      id="reviews"
      className="flex flex-col items-center w-full select-none min-h-[65vh] gap-16 bg-primary md:px-10 lg:px-16 pb-16 lg:pb-28"
    >
      <div className="text-center w-full flex flex-col gap-4">
        <p className="text-[12px] font-secondary uppercase tracking-[1px] text-yellow-400">
          Reviews
        </p>
        <h2 className="text-3xl md:text-4xl font-bold text-white font-primary">
          Customer Voices: <br />
          Hear What <span className="text-secondary">They Say</span>!
        </h2>
      </div>
      ...
      <div className="relative w-full md:w-[70%] lg:w-[50%]">
        ...
        <div
          id="prevBtn"
          className="absolute inset-y-0 left-0 z-10 hidden lg:flex items-center text-secondary"
        >
          <button
            aria-label="Previous Slide"
            className="border border-yellow-600 p-3 rounded-full hover:bg-yellow-600/20"
          >
        ...
        <div
          id="nextBtn"
          className="absolute inset-y-0 right-0 z-10 hidden lg:flex items-center text-secondary"
        >
          <button
        ...
        >
          {reviewsObj.map((review, index) => (
            <SwiperSlide
              ...
              className="transition-transform duration-500 ease-in-out transform scale-95 hover:scale-100 h-96 flex items-center"
            >
              <div className="flex flex-col gap-5 text-center px-5 rounded-lg h-full justify-center">
                <div className="rounded-full overflow-hidden w-28 h-28 md:w-32 md:h-32 mx-auto bg-secondary">
                  <img
                    src={review.img}
                    alt={`Review by ${review.name || "client"}`}
                    className="object-cover w-full h-full"
                    loading="lazy"
                  />
                </div>
